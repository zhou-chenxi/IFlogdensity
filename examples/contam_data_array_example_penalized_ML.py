...
if __name__ == '__main__':
    ...
    contam_data_array = np.arange(5., 410., 5)
    ...
    for i in random_seed_nums:
        ...
        result = eval_IF_MLlogdensity_contam_data_array(
            ...
            rel_tol_param=None,
            save_data=True,
            save_dir=f'PenML-basisn={len(start_grid_points)}-bw={bw}-kernel={kernel_type}-loglambda={log_pen_param}-contamweight={contam_weight}-plotdomain={plot_xlimit}-plotcnts={plot_cnt}-abstol={abstol}-stepsize={stepsize}-seed={i}',
            print_error=True)
